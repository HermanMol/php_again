...
// define the function for loggin errors
function hmo_php_errors($p_errcode, $p_errmsg, $p_errfile, $p_errline, $p_context)
{
	$msg = "*ERROR* "
		. date("Ymd His") 
		. "|" . $_ENV["REMOTE_ADDR"] 
	...
	if ($p_errcode <> 8) 
	{
		error_log($msg, 3, '_hmo_runtime_errors.log');
	}
}
//-----------------------------------------------------------------------------
// and set the error handler
set_error_handler('hmo_php_errors');
...
//	$link = @mysql_connect($_POST['DBSERVER'], $_POST['DBUSER'], $_POST['DBPWD']);
	$link = new mysqli($_POST['DBSERVER'], $_POST['DBUSER'], $_POST['DBPWD']);
//// Check connection
...
//echo "Connected successfully";
	...
	{
		if ($link->select_db($_POST['DBNAME'])) 
		{
...
<table style='background: #D4D0C8; border: 0;'>
	<tr>
	<td style='vertical-align: top;'><div style='width: 800px; height: 200px; overflow: hide;'>
			<form id="f1" name="f1" action="<?php echo $PHP_SELF;?>" method="post">
				<div style='background: #D4D0C8'>
				  ...
				  &nbsp;<input type="submit" name="loadrun" value="LoadRun" tabindex='12' onclick='readfile(sqlfiles.options[sqlfiles.selectedIndex].value);form.submit();return false;'>				  
				</div>
				<textarea cols='150' rows='20' wrap='soft'  
					id='sql' name='sql' tabindex='1' 
					title='SQL statement' accesskey='Q'
					onkeyup='if (event.keyCode == 120) {showit();form.submit();return false;};'
					><?php echo get_magic_quotes_gpc() ? stripslashes($_POST['sql']) : $_POST['sql']; ?></textarea>
				<textarea cols='150' rows='20' wrap='soft'  id='sql2' name='sql2' tabindex='1' style='display: none;'></textarea>
			</form>
		</div>
	</td>
	<td style='vertical-align: top;'>
		<div id='tblidx' style='width: 215px; height: 300px; overflow: auto;'>
			<table>
				<tr><th>Tables</th></tr>
				<?php
				if ($dbok)
				{
					$rs_sqlcmd = $link->query('show tables;');
					$cols = $rs_sqlcmd->field_count;
					while ($row = $rs_sqlcmd->fetch_array(MYSQLI_NUM)) 
					{
						echo "<tr>";
	...
	if ($dbok)
	{
		if ($_REQUEST['showtable'] != '?')
		{
			...
			if ($rq[0] == 'data') 
			{
				...
				$rs_sqlcmd = $link->query($sqlcmd1 . ';');
			}
			if ($rq[0] == 'ddl')
			{
				$rs_sqlcmd = $link->query('show columns from ' . $rq[1] . ';');
			}
			$cols = $rs_sqlcmd->field_count;
			$rows = $rs_sqlcmd->num_rows;
			$display_result = "<thead><tr>";
			for ($col=0; $col < $cols ; $col++ ) 
			{
	  		$display_result .= "<th>" . $$rs_sqlcmd->fetch_fields('name')[$col] . "</th>";
	  	}
		...
		else
		{
			if ($_POST['sql2'])
			{
				$sqlcmd = get_magic_quotes_gpc() ? stripslashes($_POST['sql2']) : $_POST['sql2'] ;
			}
			else
			{
			...
			if ($sqlcmd)
			{
				...
				$rs_sqlcmd = $link->query($sqlcmd) or die('Error or running query: ' . $link->error());
				if (strtoupper(substr(trim($sqlcmd),0,6))=='SELECT' || strtoupper(substr(trim($sqlcmd),0,4))=='SHOW')
				{
					$rows = $link->num_rows($rs_sqlcmd);
					if ($rows > 0)
					{
						$cols = $link->num_fields($rs_sqlcmd);
						$display_result = "<tr>";
						for ($col=0; $col < $cols ; $col++ ) 
						{
							$display_result .= "<th>" . $link->field_name($rs_sqlcmd, $col) . "</th>";
							if ($csv)
							{
								$expdata .= $fs . '"' . $link->field_name($rs_sqlcmd, $col) . '"';
								$fs = $fs_def;
							}
						}
						$display_result .= "</tr>";
						...
						$lcnt = 0;
						while ($row = $link->fetch_array($rs_sqlcmd, MYSQLi_NUM)) 
						{
							$display_result .= "<tr class='tr" . ($lcnt % 2) . "'>";
							if ($csv)
							{
				...
				{
					$rows = $link->affected_rows($link);
					$display_result = "<tr><td><br>&nbsp;&nbsp;$rows rows affected&nbsp;&nbsp;<br>&nbsp;</td></tr>";
				}
			}
		}
	}
	echo "Gevonden rijen:&nbsp;", $rows;
	echo "<div style='height: 60%; overflow: auto; background-color: white; width: 100%;'>";
